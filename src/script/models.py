...
class SalesStep(BaseModel):
    """Шаг скрипта продаж (новый формат).
    ...
    ``form`` — служебное указание, что занести в форму разговора: генератору
    и судье оно не показывается, иначе модель рапортует о нём вслух
    («записала», «зафиксировала») и делает это даже тогда, когда в форму
    ничего не легло. Форму заполняет отдельный агент по своему перечню полей.
    """
...
class RawSalesScript(BaseModel):
    """Скрипт продаж как у заказчика: идентификатор, версия, сводка и шаги.
    ...
    ``summary`` — куда ведём разговор целиком; пустая строка, если поля нет.
    """
