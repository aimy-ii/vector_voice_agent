...
class Settings(BaseSettings):
    ...
    #: Порог попыток задать шаг: сколько раз шаг уходит в генерацию без
    #: ответа клиента, прежде чем чекер закроет его. Счётчик растёт у всех
    #: шагов шапки.
    ...
    #: Потолок лестницы ожидания в ``respond_node`` (секунды): заглушки,
    #: пока фон добывает данные. По исчерпании — штатная генерация с тем,
    #: что уже есть, без обещаний сходить за данными.
    ladder_deadline_seconds: float = 5.0
