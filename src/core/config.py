...
class Settings(BaseSettings):
    ...
    # ─── Заглушка в эфир ────────────────────────────────────────────────────
    #: Предметные заглушки (город/филиал/стоимость) в эфир при реальном поиске.
    lookup_fillers_enabled: bool = True
    #: Связки речевого фона: законченные предложения, без плейсхолдеров.
    agent_bridge_fillers: list[str] = Field(
        default_factory=lambda: [
            "Так, сейчас посмотрю.",
            "Секунду, уточняю.",
            "Ага, вижу.",
    ...
    #: Через сколько молчания звучит первая связка речевого фона.
    bridge_first_delay: float = 1.5
    #: Пауза между последующими связками.
    bridge_next_delay: float = 2.0
    #: Сколько связок максимум за ход. Больше — бубнёж.
    bridge_filler_limit: int = 2
