...
class Settings(BaseSettings):
    ...
    #: Короткие связки, пока генератор ещё не начал отвечать. Каждая —
    #: законченное предложение: иначе синтез не закроет сегмент.
    ...
    #: Пауза перед следующей связкой, пока модель молчит.
    bridge_filler_delay: float = 1.2
