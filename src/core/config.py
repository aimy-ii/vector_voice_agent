...
class Settings(BaseSettings):
    ...
    #: Со скольких ходов подряд с одним ведущим шагом промпт собирается с
    #: характером «вывести человека на реплику». 0 — выключено, промпт всегда
    #: штатный.
    lead_repeat_threshold: int = 2
    ...
    #: URL справочника. В vector-infra compose перезаписывает на
    #: ``http://vector-kb:8317`` (общая сеть ``vector``). Дефолт — для
    #: локального compose этого репозитория: справочник на порту хоста,
    #: из контейнера через ``host.docker.internal`` (см. ``extra_hosts``).
    vector_kb_url: str = "http://host.docker.internal:8317"
    vector_kb_timeout: float = 2.0
    vector_kb_cache_ttl: int = 3600

    # ─── Подбор ближайшего филиала ──────────────────────────────────────────
    #: Выключатель гео-подбора: False — за координатами не ходим и блок
    #: ближайших филиалов в контексте не появляется.
    nearest_branches_enabled: bool = True
    #: Сколько филиалов просить у справочника.
    nearest_branches_limit: int = 3
    #: Радиус подбора, километры.
    nearest_branches_radius_km: float = 50.0
    #: Таймаут одного обращения к геокодеру, секунды. Геокодер — единственный
    #: поход наружу; общий таймаут справочника с двумя повторами для него
    #: слишком щедрый.
    geocode_timeout_seconds: float = 1.0
    ...
    agent_fallback: str = "Секундочку, пожалуйста."
    ...
    #: Откуда берётся скрипт: ``json`` — файл из образа, ``http`` — админка
    #: справочника. Значение задаётся в ``.env``; по умолчанию файл, поэтому
    #: выкатка образа сама по себе поведения не меняет.
    script_source: str = "json"
    #: Сколько ждать ответа админки, секунды. Держим коротким: не дождались —
    #: берём файл и ведём звонок дальше.
    script_http_timeout: float = 3.0
    #: Сколько секунд процесс считает «последнюю версию» свежей. Точные
    #: версии кэшируются навсегда — опубликованная версия неизменяема, —
    #: а вот «последняя» меняется публикацией, и её нужно перепроверять.
    #: Идущих звонков это не касается: у них версия закреплена.
    script_latest_ttl: int = 60
    #: Сколько ходов шаг может провисеть в шапке, прежде чем чекер закроет
    #: его без вердикта судьи.
    #:
    #: Страховка, а не основной механизм: закрывает по-прежнему судья, и
    #: почти всегда успевает. Но у презентационных шагов («как проходит
    #: практика», «что входит в курс») естественного сигнала закрытия от
    #: человека нет — он их просто выслушивает, — и судья держит их
    #: открытыми ход за ходом. На разборе живого звонка так висели четыре
    #: шага сразу, до восьми ходов каждый: бот возвращался к ним снова и
    #: снова, и это читается как «повторяет вопросы».
    #:
    #: Имя новое намеренно. Прежний ``STEP_ATTEMPT_LIMIT`` со значением 2
    #: означал другое и в решениях не участвовал вовсе; подставить старое
    #: значение сюда — закрыть половину сценария на втором ходу, потому что
    #: счётчик растёт у всех шагов шапки, а не только у ведущего.
    step_head_limit: int = Field(
        default=6,
        validation_alias=AliasChoices("step_head_limit", "STEP_HEAD_LIMIT"),
    )
    #: Сколько раз шаг может повести разговор, не получив ответа, прежде
    #: чем чекер закроет его без вердикта судьи.
    #:
    #: Считается по ``lead_counts``: он растёт только у ведущего шага хода,
    #: а не у всей шапки, — то есть это буквально «сколько раз человека
    #: спросили об этой теме». ``step_head_limit`` меряет другое: сколько
    #: ходов шаг просто пролежал в шапке, и до порога доходит позже.
    #:
    #: На разборе живых звонков бот трижды за разговор спрашивал про формат
    #: теории — человек всё это время задавал свои вопросы про цену и
    #: скидки. Третий заход на одну тему живой менеджер не делает: он
    #: оставляет её и идёт дальше.
    lead_give_up: int = Field(
        default=3,
        validation_alias=AliasChoices("lead_give_up", "LEAD_GIVE_UP"),
    )
    ...
    #: Сколько последних сообщений диалога идёт в укороченный промпт ожидания.
    waiting_history_limit: int = 4
    #: Сколько последних сообщений идёт в промпт живой реакции.
    filler_history_limit: int = 2
    ...
    #: С какого числа реплик диалога звать агента конца разговора.
    #: В начале звонка прощаться не с чем, лишний вызов модели ни к чему.
    farewell_min_messages: int = 5
    ...
    #: Запасной разбор ``thread_id``: если в ``configurable`` нет ``call_id``
    #: и идентификатор треда оканчивается этим суффиксом — отрезать его.
    #: Бот так треды не именует (лайв-тред — ``uuid5`` от ``"{room}#live"``) и
    #: всегда передаёт ``call_id`` явно; ветка нужна только как страховка.
