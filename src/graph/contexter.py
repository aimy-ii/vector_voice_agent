...
Исполняется либо фоновым графом ``vector_contexter`` из очереди (штатный
путь), либо синхронно внутри служебного прохода (запасной путь при
недоступной очереди); поведение разбора в обоих путях одно.
