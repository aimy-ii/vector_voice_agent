...
#: Многоточие сюда не входит: оно заменяется на точку (см. ``_ensure_terminal_punct``).
_TERMINAL_PUNCT = ".!?,:;"
...
def pick_ack(*, spoken: Sequence[str] = ()) -> str:
    """Короткий отклик в начало хода: «Так, секунду.», «Угу, минутку.».
...
def pick_filler(subject: str | None, *, spoken: Sequence[str] = ()) -> str:
    ...
    Если после подстановки фраза оканчивается многоточием — оно заменяется
    на точку; иначе при отсутствии конечного знака добавляется точка, чтобы
    заглушка не склеивалась с репликой генератора.
