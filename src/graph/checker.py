...
def checker_system_prompt(*, asked: bool) -> str:
    """Системная часть промпта чекера.

    Args:
        asked: вопрос шага уже задавали (счётчик попыток больше нуля).
    ...
    if asked:
        lines.append(
            "Вопрос шага уже звучал. Закрывает только ответ клиента по"
            " существу, прозвучавший после этого вопроса; всё остальное"
            " оставляет шаг открытым."
        )
    else:
        lines.append(
...
def _asked_line(attempts: int) -> str:
    """Строка блока шага: задавали ли вопрос и сколько раз."""
    if attempts > 0:
...
class LlmCheckerClient:
    ...
    async def judge(
        ...
        *,
        ...
        system = checker_system_prompt(asked=attempts > 0)
        asked = _asked_line(attempts)
        if isinstance(step, SalesStep):
            human_parts = [
