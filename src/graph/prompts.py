...
SPEECH_RULES: tuple[str, ...] = (
    ...
    "Реплика всегда заканчивается ходом к собеседнику — вопросом по делу, который двигает разговор, или предложением выбрать из названного. Заканчивать реплику ничем нельзя. Пустая проверка в конце запрещена: вопрос, на который любой ответ не меняет следующий шаг (продолжим / что скажете / вас устраивает / хорошо и любые переделки) — это галочка в скрипте, а не ход разговора. Плохо: «Курс под ключ, доплат не будет. Продолжим?» Плохо: «Курс под ключ, доплат не будет.» Хорошо: «Курс под ключ, доплат не будет. Теорию удобнее очно или в приложении?» Исключение одно: реплика ожидания, когда данные ещё готовятся — ход к человеку не нужен, бот сам продолжит следующей репликой.",
...
def naturalness_block(*, ask_for_move: bool, pending_only: bool = False) -> str:
    ...
    lines.extend(
        [
            ...
            "названного. Заканчивать реплику ничем нельзя. Пустая проверка в "
            "конце запрещена: вопрос, на который любой ответ не меняет "
            "следующий шаг. Примеры запрещённого признака: продолжим, "
            "что скажете, вас устраивает, хорошо — и любые переделки. "
...
def _describe_step(
    ...
    *,
    ...
    Args:
        ...
        heading: заголовок строки («Текущий шаг» / «Висящий шаг»).
        attempts: сколько раз шаг уже брали.
...
def steps_block(
    ...
    *,
    ...
    """Описывает шапку: ведущий текущий и остальные висящие.
    ...
    focus_rule = (
        "Реплика строится по первому шагу шапки (текущему). Содержание шагов, "
        "стоящих дальше, не рассказывать, пока разговор до них не дошёл — "
        "иначе придётся повторять их заново, когда очередь дойдёт. "
    ...
    for index, step in enumerate(steps):
        ...
        lines.extend(
            _describe_step(
                ...
                profile,
                facts,
                heading="Текущий шаг" if is_lead else "Висящий шаг",
                attempts=count,
                context_text=context_text,
                is_lead=is_lead,
                is_new=is_new,
...
def step_block(
    ...
    *,
    ...
    Args:
        ...
        profile: профиль.
        facts: факты хода.
        next_step: следующий шаг или None; в шапке — висящий, не текущий.
        attempts: счётчики попыток.
