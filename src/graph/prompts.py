...
def filler_spoken_block(
    ...
    *,
    ...
    if not text:
        return ""
    # Подтверждения и междометия уже звучали в отклике — повтор = заикание.
    no_echo = (
        "Реплика не начинается с подтверждающего слова или междометия: они уже "
        "прозвучали в отклике, повтор звучит как заикание. Сразу к сути шага. "
        "Плохо: отклик «Угу, минутку.» → «Понятно, Андрей. Права получаете впервые?». "
        "Хорошо: отклик «Угу, минутку.» → «Права получаете впервые или уже был опыт?»."
    )
    if filler_subject:
        return (
            ...
            f"не делать, второго вступления не начинать. {no_echo}"
        )
    return (
        f"В эфир уже ушла фраза: «{text}». Продолжать с места, без «добрый день» "
        f"и без повторного «так»; подводка к теме при этом уместна. {no_echo}"
    )
