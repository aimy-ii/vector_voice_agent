...
SPEECH_RULES: tuple[str, ...] = (
    ...
    "Если среди задач есть вопрос по делу — задать его в конце реплики. Если задач только на рассказ — закончить фактом, без пустого хвоста. Пустая проверка в конце запрещена: вопрос, на который любой ответ не меняет следующий шаг (продолжим / что скажете / вас устраивает / хорошо и любые переделки) — это галочка в скрипте, а не ход разговора.",
...
def naturalness_block(*, ask_for_move: bool, pending_only: bool = False) -> str:
    ...
    lines.extend(
        [
            ...
            "- Реплика заканчивается либо вопросом по делу, который двигает "
            "разговор дальше, либо ничем — просто фактом. Пустая проверка в "
            ...
            "очно или в приложении?». Тоже хорошо: «Обучение под ключ, "
            "доплат не будет.». Одинаковый конец не повторяется: если в "
            "прошлой реплике был вопрос-связка, в следующей его формулировка "
            "другая своими словами, каждый раз по-разному, либо его нет "
            "вовсе. Плохо (канцелярская пластинка): одна и та же пустая "
            "проверка после каждой реплики.",
