...
_HARD_FACT_BAN = (
    "Жёсткий запрет: об услуге говорить только то, что есть в переданных "
    "данных. Цифры, цены, сроки, адреса, даты, названия — оттуда же. "
    "Условия обучения — с какого возраста берут, какие документы нужны, "
    "кого принимают, что будет при незачёте, что с деньгами, налогами и "
    "возвратом — это правила школы, а не общие знания, и перечень тут не "
    "закрытый: любое утверждение о том, как устроено обучение и оплата, "
    "берётся из данных. Чего в данных нет, того для разговора не "
    "существует: об этом не рассказывают по своей воле и не приводят как "
    "довод, даже когда это правда для мира вообще. Спросили о таком — "
    "сказать, что уточнишь и вернёшься с ответом, и не придумывать."
)


#: Короткая версия запрета — для заглушек.
#:
#: Заглушка ничего не рассказывает: она держит паузу, пока готовятся
#: данные. Полный запрет там вдвое длиннее самой сборки, а разбирать в
#: нём нечего — фактов в заглушке нет по определению.
_HARD_FACT_BAN_SHORT = (
    "Жёсткий запрет: об услуге не говорить ничего сверх переданных "
    "данных — ни цифр, ни условий, ни доводов."
)
