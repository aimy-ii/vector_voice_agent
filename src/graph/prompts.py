...
def naturalness_block(*, ask_for_move: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "- Если шаг предполагает проверочный вопрос — задавай его целиком и "
            "осмысленно, не обрезай до куцего огрызка. Плохо: «Рассказывать про "
            "практику?». Хорошо: «Как вам в целом такой подход?».",
        ]
    )
    return "\n".join(lines)
...
def _describe_step(
    ...
    *,
    ...
    if step.check_question:
        lines.append(
            "Проверочный вопрос (обязателен, задай целиком и по смыслу, "
            f"не сокращай и не заменяй своим):\n«{step.check_question}»"
        )
    if step.options:
        lines.append(
