...
_PULL_TASK_SHORT = (
    "Задача этого хода одна: растормошить человека и втянуть его обратно в "
    "разговор. Предыдущая реплика вопроса не содержала, разговор повис, и "
    "ждать ответа нельзя — разговор ведём мы."
)
...
#: Второе главное требование режима: стоит сразу за требованием вопроса, а не
#: в хвосте списка. Пока запрет шёл четвёртым, после потолка длины, модель
#: считала его необязательным и начинала добивку пересказом.
_PULL_NO_REPEAT = (
    "ВТОРОЕ ГЛАВНОЕ: последнюю мысль не повторять и не пересказывать другими "
    "словами, уже названное заново не перечислять. Человек это слышал — потому "
    "и молчит."
)

#: Приём, которым модель обходила запрет повтора: пересказ прятался в связке
#: на входе, а вопрос на конце делал реплику формально исправной. Пример взят
#: из боевого звонка — без него запрет читается как повтор предыдущего.
#: Образцовой реплики здесь нет намеренно: годный образец модель зачитывает
#: дословно, поэтому «хорошо» описано, а не процитировано.
_PULL_NO_PICKUP = (
    "Реплика не начинается с подхвата собственной последней мысли. Связки "
    "«поняла», «хорошо», «да, отправлю», «как и сказала», «итак» в начале — "
    "тот же пересказ под новой шапкой, и вопрос в конце его не оправдывает. "
    "Плохо: сказала «Хорошо, отправлю информацию в Max», а добивка звучит "
    "«Поняла, отправляю всё в Max. Подскажите, медсправка уже есть?». "
    "Хорошо: начать сразу с того, чего в разговоре ещё не было, — без шапки "
    "про уже сказанное."
)

#: Чем растормошить — решает модель по всему разговору. Разбора случаев здесь
#: нет: он решал за модель то, что она видит лучше, и подсказывал самый
#: дешёвый ход — вернуться к текущему шагу и пересказать сказанное.
_PULL_OWN_CHOICE = (
    "Чем растормошить — выбор твой, готовых предписаний тут нет. Прочитай весь "
    "разговор целиком: где он повис, что человеку уже сказано и обещано, о чём "
    "он спрашивал сам и что осталось нерешённым, — и возьми то, что сдвинет "
    "разговор с места. Остаться на текущем шаге или перевести разговор на "
    "другую тему — тоже твоё решение."
)

#: Граница добивки по смыслу вместо потолка в двадцать пять слов. Потолок
#: зажимал модель между «скажи новое» и «уложись в счёт», и самым дешёвым
#: выходом из рамки оказывался подхват своей же мысли с вопросом на конце.
_PULL_SCOPE = (
    "Объём — ровно столько, сколько нужно, чтобы человек снова заговорил: одна "
    "мысль и вопрос к ней. Новой презентацией добивка при этом не становится — "
    "три темы подряд, перечисления через запятую и пересказ всего, о чём "
    "договорились, к разговору не возвращают. Вопрос идёт последним."
)
...
def build_pull_messages(
    ...
    *,
    ...
    человек после неё молчит. Нужна одна добивка с вопросом, а не полный ход
    продажи, поэтому в промпт идут только роль, требования режима, ведущий шаг,
    уже добытый контекст и форма разговора. Полного перечня правил речи, сводки
    сценария и шапки из нескольких шагов здесь нет: для одной реплики они только
    разбавляют главное.

    История разговора уходит целиком, как в полной сборке: чем растормошить,
    модель выбирает сама, а по хвосту из нескольких сообщений не видно, что
    уже сказано и обещано, — и добивка повторяла саму себя.

    Требования идут в порядке важности: вопрос в конце и запрет пересказа —
    первыми, потом свобода выбора темы и границы объёма.
