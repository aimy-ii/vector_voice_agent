...
SPEECH_RULES: tuple[str, ...] = (
    ...
    "Не обещать сходить за данными и вернуться. Если данных нет — не сообщать "
    "об этом и не извиняться: подробности и точные цифры придут в мессенджере, "
    "сейчас продолжать разговор. Мессенджер ещё не выбран — спросить, в каком "
    "удобнее.",
...
def naturalness_block(*, ask_for_move: bool, pending_only: bool = False) -> str:
    ...
    lines.extend(
        [
            ...
            "- Имя — только имя, без отчества. Звучит один раз при знакомстве; "
            "дальше только в ключевых местах: при закреплении места и при "
            "прощании. Две реплики подряд с именем — ошибка. Начинать реплику "
            "с имени по умолчанию нельзя.",
...
#: Нет данных — не признавать пробел, а уводить подробности в переписку.
#: Одна формулировка для коротких сборок; в SPEECH_RULES — та же мысль.
_MISSING_TO_MESSENGER = (
    "Если данных нет — не сообщать об этом и не извиняться: подробности и "
    "точные цифры придут в мессенджере, сейчас продолжать разговор. "
    "Мессенджер ещё не выбран — спросить, в каком удобнее."
)
...
def dynamic_status_block(*, status: str, searching_retry: bool = False) -> str:
    ...
    if status == DYN_MISSING:
        return (
            "По нужному факту в данных ничего нет. Не сообщать об этом клиенту "
            "и не извиняться — подробности увести в мессенджер и продолжить "
            "разговор, не выдумывать."
        )
    if status == DYN_READY:
        return ""
    return ""
...
def unknown_block(script: CompiledScript) -> str:
    ...
    """
    return (
        "Если точного ответа нет — не придумывать, не сообщать об отсутствии "
        "данных и не извиняться. Увести в переписку примерно так и продолжить "
        f"разговор: «{script.params.unknown}». "
...
def build_filler_messages(
    ...
    *,
    ...
    system = (
        ...
        f"{_MISSING_TO_MESSENGER} {_HARD_FACT_BAN}"
    )
...
def build_silence_messages(
    ...
    *,
    ...
    lines.append(_MISSING_TO_MESSENGER)
    lines.append(_HARD_FACT_BAN)
    if step is not None:
        name = getattr(step, "name", None) or getattr(step, "goal", None) or step.id
