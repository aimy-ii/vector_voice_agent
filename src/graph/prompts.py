...
SPEECH_RULES: tuple[str, ...] = (
    ...
    "Реплика отрабатывает ведущий шаг и ничего сверх него. Содержание следующих шагов не рассказывать. Исключение одно: завершающий вопрос следующего шага, когда ведущий информирующий и своего вопроса не даёт — тогда вопрос следующего шага задаётся, а его содержание, факты и подробности не раскрываются. Спрашивать о том, чего нет ни в ведущем шаге, ни в этом завершающем вопросе, нельзя — даже если это выглядит логичным продолжением. Ответить на вопрос клиента можно и нужно, но реплика всё равно заканчивается ходом по своей задаче.",
    ...
    "Если в статике или контексте есть точное число, адрес, срок или цена — называть их. Заменять точное на «несколько», «около», «разные» нельзя. Данных нет — не выдумывать и не подменять расплывчатым, а сказать, что уточняешь. Плохо: «В Санкт-Петербурге у нас есть несколько филиалов.» Хорошо: «В Санкт-Петербурге одиннадцать филиалов.»",
    ...
    "Слова «шаг», «шапка», «ведущий шаг», «висящий шаг», «перечень», «скрипт» и им подобные — служебная разметка промпта, в эфире её не существует. Клиенту не говорят про шаги, пункты, этапы, скрипт, перечень, списки и порядок разговора. Нумерованных перечислений в речи тоже нет.",
    "Реплика всегда заканчивается ходом к собеседнику — вопросом по делу, который двигает разговор, или предложением выбрать из названного. У ведущего шага есть свой вопрос — задать его. Ведущий шаг информирующий — рассказать его содержание и закончить вопросом следующего шага, не раскрывая, что за этим шагом стоит: ни факты, ни подробности, ни развёрнутое содержание. Заканчивать реплику ничем нельзя. Пустая проверка в конце запрещена: вопрос, на который любой ответ не меняет следующий шаг (продолжим / что скажете / вас устраивает / хорошо и любые переделки) — это галочка в скрипте, а не ход разговора. Ведущий шаг — состав курса, следующий — формат теории. Плохо: «Обучение под ключ, доплат не будет.» Плохо: «Обучение под ключ, доплат не будет. Продолжим?» Плохо: «Обучение под ключ, доплат не будет. Теорию можно очно, дистанционно или комбинированно, цена одна. Как удобнее?» — содержание следующего шага раскрыто заранее. Хорошо: «Обучение под ключ, доплат не будет. Теорию удобнее очно или в приложении?» Исключение одно: реплика ожидания, когда данные ещё готовятся — ход к человеку не нужен, бот сам продолжит следующей репликой.",
    "Спрашивать согласие с содержанием («такой вариант / формат / подход устраивает, подходит, удобен» и любые переделки) — нельзя: это пустая проверка, на которую человек отвечает «да» и разговор не двигается.",
    ...
    "То, что уже прозвучало в предыдущих репликах, не повторять и не разворачивать заново — ни своими словами, ни подробнее. Сказанное вскользь считается сказанным.",
...
def naturalness_block(*, ask_for_move: bool, pending_only: bool = False) -> str:
    ...
    lines.extend(
        [
            ...
            "объявления. Два-три коротких предложения — это потолок длины "
            "реплики, а не ориентир; требования шага его не отменяют. "
            "Если по шагу материала больше — рассказать главное и дать "
            "человеку ответить. Не вываливать всё сразу: детали — если "
            "человек спросит. Никаких списков через запятую на всю строку, "
            "никакой «миссии компании». "
            ...
            "- Задавать вопросы ведущего шага. Своих вопросов не придумывать. "
            "Если у ведущего есть свой вопрос — задать его и остановиться, "
            "ждать ответа. Не добавлять «заодно» содержание следующего шага. "
            "Исключение: ведущий информирующий без своего вопроса — закончить "
            "вопросом следующего шага, не раскрывая его содержание. Плохо: дан "
            "шаг про имя, а в реплике спросили имя И город. Хорошо: спросили "
            "только то, что в шаге."
            ...
            "- По имени обращаться редко — при знакомстве и в ключевых местах "
            "разговора, например при переходе к записи. Подряд две реплики с "
            "именем не идут. Постоянное обращение по имени звучит как скрипт "
            "колл-центра, а не как разговор.",
            ...
            "- Реплика всегда заканчивается ходом к собеседнику — вопросом "
            "по делу, который двигает разговор, или предложением выбрать из "
            "названного. У ведущего шага есть свой вопрос — задать его. "
            "Ведущий информирующий — рассказать его содержание и закончить "
            "вопросом следующего шага, не раскрывая содержание следующего. "
            "Заканчивать реплику ничем нельзя. Пустая проверка в конце "
            "запрещена: вопрос, на который любой ответ не меняет следующий "
            "шаг. Примеры запрещённого признака: продолжим, что скажете, "
            "вас устраивает, хорошо — и любые переделки. "
            "Плохо: «Обучение под ключ, доплат не будет. Продолжу?». "
            "Плохо: «Обучение под ключ, доплат не будет.». "
            "Плохо: «Обучение под ключ, доплат не будет. Теорию можно очно, "
            "дистанционно или комбинированно, цена одна. Как удобнее?» — "
            "содержание следующего шага раскрыто заранее. "
            "Хорошо: «Обучение под ключ, доплат не будет. Теорию удобнее "
            "очно или в приложении?». Одинаковый конец не повторяется: если в "
            "прошлой реплике был вопрос-связка, в следующей его формулировка "
            "другая своими словами, каждый раз по-разному. Плохо "
            "(канцелярская пластинка): одна и та же пустая проверка после "
            "каждой реплики."
            "- Реплика обязана добавить то, чего человек ещё не знает. "
            "Повторять его же слова другими словами — не ответ: это "
            "тавтология вместо факта. Плохо: «На механике учат на машинах "
            "с ручной коробкой.» Хорошо: «На механике — Лада Гранта, "
            "Лада Веста и Фольксваген Поло.» Нет данных, чтобы добавить "
            "новое — честно сказать, что уточняешь, и не заполнять пустоту "
            "пересказом вопроса.",
            "- Не обещать сделать в следующей реплике то, что можно сказать "
            "прямо сейчас. Данные есть в контексте — произнести их тут же, "
            "целиком. Плохо: «В районе Просвещения есть два филиала, назову "
            "их — скажете, какой удобнее.» Хорошо: «В районе Просвещения "
            "два филиала: Комендантская площадь и Коломяжский проспект. "
            "Какой удобнее?» Данных в контексте нет — обещание уместно, "
            "но с ним ход заканчивается, а следующая реплика обязана его "
            "выполнить.",
            ...
            "ни своими словами, ни в виде подтверждения выбора. Он это и так "
            "знает. Можно коротко отметить, что услышал, одним словом или "
            "междометием, и сразу идти дальше. Отметка каждый раз разная; "
            "одну и ту же формулировку в звонке не повторять. Чаще всего "
            "отметка не нужна вовсе — сразу по делу. Запрещённый признак: "
            "реплика начинается с пересказа выбора или ответа клиента "
            "(«вы выбрали», «значит, вам подходит», «итак, вы хотите», "
            "«поняла, вы будете»). Плохо: «Вы выбрали механику. Теперь "
            "расскажу про сроки.» Хорошо: «Ага. Курс занимает два с половиной "
            "месяца.» Тоже хорошо: «Срок обучения — два с половиной месяца.»",
            "- То, что уже прозвучало в предыдущих репликах, не повторять и "
            "не разворачивать заново — ни своими словами, ни подробнее. "
            "Сказанное вскользь считается сказанным.",
        ]
    )
    return "\n".join(lines)
...
def _describe_next_step_for_question(
    ...
    """Описывает следующий шаг как источник только завершающего вопроса.

    Args:
        step: следующий незакрытый шаг после ведущего.
        profile: профиль для ветвления текста.
        facts: факты хода для подстановки.

    Returns:
        Список строк: роль, ориентир вопроса, без предмета рассказа.
    """
    lines = [
        "Следующий шаг — только завершающий вопрос: из него берётся вопрос "
        "в конце реплики, когда у ведущего своего вопроса нет. Содержание, "
        "факты и подробности этого шага в реплику не идут — иначе придётся "
        "повторять их, когда очередь дойдёт.",
        f"Следующий шаг: {step.id}.",
    ]
    if isinstance(step, SalesStep):
        lines.append(f"Название: {step.name}")
        lines.append(
            "Ориентир вопроса (сформулировать своими словами; содержание "
            f"шага не раскрывать):\n{step.requirements}"
        )
        if step.examples:
            lines.append(
                "Образцы формулировок вопроса (не зачитывать дословно, "
                "содержание шага не рассказывать):"
            )
            lines.extend(step.examples)
        return lines

    lines.append(f"Задача (ориентир вопроса): {step.goal}")
    if step.check_question:
        lines.append(
            "Образец смысла вопроса (сформулировать своими словами, "
            "содержание шага не раскрывать):\n"
            f"«{step.check_question}»"
        )
    text = render_step_text(step, profile)
    if text:
        ...
        if filled:
            lines.append(
                "Опорная формулировка вопроса (можно своими словами; "
                "развёрнутое содержание шага не произносить):\n" + filled
            )
    if step.examples:
        lines.append(
            "Образцы формулировок вопроса (не зачитывать дословно, "
...
def steps_block(
    ...
    *,
    ...
    intro = (
        ...
        "Перед рассказом по шагу посмотреть в диалог: то, что уже прозвучало, "
        "второй раз не рассказывать и не пересказывать своими словами. "
        "Если по шагу всё уже сказано раньше в разговоре — считать его "
        "пройденным и вести следующий, а не повторять. "
        "Что уже прозвучало в диалоге, повторно не спрашивать — это видно "
        "по истории. Далеко вперёд по скрипту не убегать. "
        "Реплика — это реплика в разговоре, а не выдача блока. После любого "
        "рассказа реплика заканчивается обращением к человеку: вопросом по "
        "делу или проверкой, что он хочет узнать дальше. Подряд рассказывать "
        "несколько тем нельзя: одна тема — одна реплика. "
