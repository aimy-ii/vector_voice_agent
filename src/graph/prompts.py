...
#: тема и требования подаются штатно, меняется только задача хода. Раздел
#: «ЕЩЁ НЕ ЗАКРЫТО» по названию не упоминается: при одном шаге в шапке его
#: в промпте нет, и ссылка уходила бы в пустоту.
PULL_TASK: str = (
