...
#:
#: Две прошлые версии набирали запреты («не повторяйся», «не подхватывай
#: начало», «выбирай тему сам») и получали реплику, которая формально не
#: нарушает ни одного: пересказ собственной последней мысли под новой шапкой
#: с вопросом на конце. Запрет описывает брак, но не говорит, что делать
#: вместо него, и модель берёт самый дешёвый ход — тот, откуда только что
#: пришла. Повтор снимается не запретом, а первым шагом порядка: прочитав
#: разговор целиком, модель видит, что уже сказано, и второй шаг уводит её
#: к незакрытому, а не к последней собственной фразе.
#:
#: Запреты про подхват начала и про объём свёрнуты внутрь третьего шага:
#: там они читаются как описание нужной реплики, а не как ещё два пункта
#: в списке нельзя.
_PULL_THINKING = (
    ...
    "которого продолжают.\n"
    "3. Скажи то, чего в этом разговоре ещё не было, и закончи вопросом. "
    "Начинай сразу с нового: «поняла», «как и сказала», «итак», «хорошо, "
    "отправлю» в начале — это шапка про уже сказанное, и после неё реплика "
    "начинается с повтора. Одна мысль и вопрос к ней — этого достаточно; "
    "пересказывать, к чему пришли, и выкладывать три темы подряд не нужно."
)

#: Первое из двух требований к готовой реплике. Сформулировано через то, чем
#: вопрос обязан быть, и перечисляет пустые формы: без этого перечня модель
#: закрывала реплику проверкой понимания и считала требование выполненным.
_PULL_QUESTION = (
...
#: Врезка перед блоком текущего шага. Сам блок конкретнее любого рассуждения
#: и перетягивает ход на себя: предыдущая реплика бота выросла из этого же
#: шага, и без врезки модель отрабатывает его второй раз подряд — отсюда
#: пересказ. Врезка подчиняет шаг порядку мышления: это направление, а не
#: текст хода, и отработанное повторно не отрабатывают.
_PULL_STEP_INTRO = (
    "Куда разговор идёт дальше по делу. Это ориентир к третьему шагу "
    "рассуждения, а не текст, который надо произнести сейчас: предыдущая "
    "твоя реплика выросла отсюда же. Видно по разговору, что это уже "
    "сказано, — значит, оно закрыто, и говорить надо о другом."
)
...
def build_pull_messages(
    ...
    *,
    ...
    ход продажи, поэтому в промпт идут только роль, порядок мышления, два
    требования к готовой реплике, ведущий шаг, уже добытый контекст и форма
    разговора. Полного перечня правил речи, сводки сценария и шапки из
    нескольких шагов здесь нет: для одной реплики они разбавляют главное.

    Промпт построен вокруг последовательности действий, а не вокруг перечня
    нельзя: прочитать разговор, найти незакрытое, сказать новое с вопросом.
    Перечень запретов эту задачу не решал — модель выдавала реплику, формально
    не нарушающую ни одного пункта: пересказ своей последней мысли с вопросом
    на конце. Требований осталось два: вопрос в конце и запрет называть факты
    вне переданных данных.
