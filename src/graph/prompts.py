...
def unknown_block(script: CompiledScript) -> str:
    ...
    return (
        ...
        "Наличие филиала в конкретном районе, у конкретного метро или на "
        "конкретной улице утверждать можно только если этот филиал пришёл в "
        "фактах или контексте. Названного клиентом района в данных нет — так и "
        "сказать и предложить ближайшие из тех, что есть. Придумывать филиал "
        "по названию района запрещено."
    )
