...
SPEECH_RULES: tuple[str, ...] = (
    ...
    "Реплика заканчивается движением разговора вперёд. Если среди задач есть вопрос — задать его. Если задача только рассказать, а вопроса среди задач нет — закончить самим фактом, без вопроса. Выдумывать вопрос, чтобы чем-то закончить, нельзя.",
    "Проверок после рассказа не бывает. Спрашивать «такой вариант / формат / подход устраивает, подходит, удобен» и любые их переделки — нельзя ни в каком виде: это пустой вопрос, на который человек отвечает «да» и разговор не двигается. Рассказали — переходите к следующей задаче.",
