...
#:
#: Без шапки шагов модели не из чего брать тему: она видит только
#: повисшую реплику и продолжает её. Порядок сначала читает разговор,
#: затем прямо указывает источник темы — незакрытые шаги или следующий.
#: Шапка ниже этому подчинена: это меню тем, а не текст хода.
#:
#: Запрет повтора и запрет подхвата снова отдельные пункты рядом с
#: вопросом: свёртка внутрь рассуждения не удержала — добивки из звонка
#: начинались с «супер, тогда», «без проблем», «поняла, тогда».
_PULL_THINKING = (
    ...
    "2. Найди, на чём он повис. Вопрос, оставшийся без ответа. Мысль, "
    "которую не договорили. Обещанное и не выполненное. Тема, которую "
    "человек поднял, а разговор ушёл в сторону. Это и есть место, с "
    "которого продолжают.\n"
    "3. Тему для реплики бери из шагов сценария ниже — из незакрытых или "
    "из следующего. Посторонних тем не бывает: всё, о чём говорим, — либо "
    "шаг сценария, либо то, что человек сам поднял. Разговор продолжается "
    "с того места, где повис, и движется к тому, что ещё не сделано. "
    "Выдумывать тему нельзя. Если шагов сценария ниже нет — значит, по "
    "сценарию всё сделано: тему бери из самого разговора.\n"
    "4. Скажи то, чего в этом разговоре ещё не было, и закончи вопросом. "
...
#: Запрет подхвата собственной мысли — отдельный пункт рядом с вопросом.
#: Примеры — три добивки из боевого звонка: каждая начинается с шапки
#: про уже сказанное, после которой идёт повтор.
_PULL_NO_CATCHUP = (
    "Реплика не начинается с подхвата собственной последней мысли: "
    "«супер, тогда», «без проблем», «поняла, тогда», «хорошо, жду» — "
    "это шапка про уже сказанное, после которой идёт повтор. "
    "Из звонка: после «Тогда обучение закрепляем за этим офисом. "
    "В течение дня скину в Max инструкцию…» нельзя «Супер, тогда "
    "закрепляю за Вами филиал на Коломяжском. Для оформления понадобится "
    "фото паспорта…»; после «Хорошо, жду Вашего сообщения в Max, когда "
    "будете готовы…» нельзя «Без проблем, жду от Вас документы, когда "
    "будет удобно…»; после «Поняла, не тороплю. Как будете готовы — "
    "напишите в Max…» нельзя «Поняла, тогда ориентируюсь на Ваше "
    "сообщение…»."
)
...
def build_pull_messages(
    ...
    *,
    ...
    найти незакрытое, взять тему из шагов сценария, сказать новое с вопросом.
    Запрет повтора и запрет подхвата — отдельные требования к готовой реплике,
    рядом с вопросом.
