...
def format_missing(place: str) -> str:
    ...
    Просит ориентир того вида, который геокодер умеет искать (улица, метро,
    район), и прямо закрывает повторную попытку назвать здание — иначе
    разговор ходит по кругу. О технической неудаче вслух не сообщаем.
    ...
    return (
        ...
        "нему не подобраны.\n"
        "Адрес по этому месту не называть и о технической неудаче не сообщать.\n"
        "Попросить другой ориентир — улицу, можно с номером дома, станцию метро "
        "или район города.\n"
        "Здания, торговые центры, храмы, парки и памятники по координатам не "
        "находятся: второй раз их не просить.\n"
        "Один вопрос, без извинений, и дальше по разговору."
    )
...
async def lookup_nearby(
    ...
    *,
    ...
    # В справочник уходит нормализованное место, а не исходная фраза.
    # Геокодер не терпит ни одного ведущего слова: «метро Пионерская» он
    # находит, «у метро Пионерская» и «ближайшее метро Пионерская» — уже
