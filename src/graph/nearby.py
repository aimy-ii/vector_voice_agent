...
#: «Уралмаш РЯДОМ С МЕТРО ПРОСПЕКТ КОСМОНАВТОВ».
#:
#: Составные предлоги стоят раньше простых: у «рядом с» хвост нужнее, чем
#: у голого «с» внутри него.
_INNER_PLACE_MARKERS: tuple[str, ...] = (
    " рядом с ",
    " недалеко от ",
    " в ",
    " на ",
...
def place_queries(place: str) -> list[str]:
    ...
    За каждой частью идёт её хвост после предлога: «торгового дома Кит в
    Приморском районе» находится по «приморском районе». Хвост берётся у
    части, а не у целой фразы, — иначе он перешагнёт запятую и склеит два
    разных ориентира в один запрос, по которому не найдётся ничего.
    ...
    def offer_tail(text: str) -> None:
        """Добавляет хвост части после предлога, если он там есть."""
        for marker in _INNER_PLACE_MARKERS:
            position = f" {text} ".rfind(marker)
            if position != -1:
                offer(f" {text} "[position + len(marker) :])
    ...
    if len(parts) == 1:
        offer_tail(whole)
    # Хвост берём у каждой части, а не у целой фразы: в «торгового дома
    # кит в приморском районе, метро пионерское» хвост целого — это
    # «приморском районе, метро пионерское», по нему геокодер не находит
    # ничего, а по «приморском районе» находит район с первой попытки.
    # Хвост идёт сразу за своей частью: запросов немного, и потратить их
    # надо на разные ориентиры, а не на разные обрезки одного.
    for part in parts:
        offer(part)
        offer_tail(part)
