...
def test_v4_branch_examples_называют_адрес(script_v4):
    """Образцы шага филиала показывают названный адрес, а не «ближайший офис».

    Требование шага велит назвать ближайший филиал, а все три образца
    показывали обратное: «От ближайшего к Вам офиса», «Есть офис совсем
    рядом», «Если этот адрес не подходит». Модель копирует форму образца,
    и на прогонах бот отвечал «подберу филиал поближе» — адреса человек
    так и не слышал, хотя в контексте он лежал.
    """
