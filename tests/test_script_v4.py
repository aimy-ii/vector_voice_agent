...
def test_v4_закрытие_discount_check_фиксирует_ответ_по_категории(script_v4):
    """Закрытие discount_check: шаг закрывает любой ответ по льготной категории.

    Раньше выходом была оговорка «льготных категорий в данных нет». Скидки в
    справочнике появились, поэтому шаг закрывает сам ответ человека — и «да»,
    ...
    assert "студент, школьник, именинник, молодая мама" in req
