...
def test_отбор_под_собеседника_вслух_не_называется(script):
    ...
    # Новое правило стоит в конце: индексы прежних правил закреплены
    # тестами, и вставка в середину сдвинула бы половину из них.
    assert "отбор под собеседника" in SPEECH_RULES[-1].lower()
