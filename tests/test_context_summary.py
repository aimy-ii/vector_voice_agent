...
def test_format_city_static_регресс_без_скидок_прежние_строки():
    ...
    # Срок называется словом: на «сколько длится обучение» бот отвечал
    # «уточню при оформлении», хотя срок стоял у него перед глазами —
    # строка читалась как перечень категорий, а не как срок.
    assert "Категории и сроки: B: срок обучения 2 месяца, набор каждые 15 дней." in text
    assert (
