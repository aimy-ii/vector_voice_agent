...
def test_условия_обучения_тоже_под_запретом_выдумок() -> None:
    """Возраст, документы и условия — правила школы, а не общие знания.

    На прогоне мать спросила, можно ли учиться сыну до восемнадцати, и бот
    в четырёх звонках из пяти ответил, что начать можно сейчас, а к
    экзамену должно исполниться восемнадцать. В справочнике города про это
    нет ничего: утверждение взято из общих знаний модели и сказано матери
    про её несовершеннолетнего ребёнка.
    """
    from graph.prompts import _HARD_FACT_BAN

    lowered = _HARD_FACT_BAN.lower()
    assert "с какого возраста" in lowered
    assert "правила школы" in lowered
    assert "не придумывать" in lowered
