...
from graph.history import agent_texts, text_of
...
def test_реплика_прозвучала_целиком_история_не_меняется():
    ...
    out = reconcile_last_agent(entries, aired=["Здравствуйте"])
    assert out == entries
    assert out is not entries


def test_реплика_не_прозвучала_последняя_запись_бота_удалена():
    """В снимке другая реплика — последняя запись бота убрана, более ранние на месте."""
    earlier = TranscriptEntry(entry_id="c:1:0", role=ROLE_CLIENT, text="Алло")
    entries = [
        earlier,
        TranscriptEntry(entry_id="a:1:1", role=ROLE_AGENT, text="Здравствуйте"),
    ]
    out = reconcile_last_agent(entries, aired=["Другая фраза"])
    assert out == [earlier]
...
def test_снимок_с_текстом_предыдущей_реплики_удаляет_последнюю():
    ...
    out = reconcile_last_agent([first, second], aired=["Первая"])
    assert out == [first]
...
def test_частичная_доставка_урезает_до_префикса():
    ...
    out = reconcile_last_agent([entry], aired=["Добрый день"])
    assert len(out) == 1
...
def test_последняя_запись_клиента_не_меняется():
    ...
    out = reconcile_last_agent(entries, aired=["Привет"])
    assert out == entries


def test_пустая_история_остаётся_пустой():
    """Пустой список — пустой результат."""
    assert reconcile_last_agent([], aired=["что угодно"]) == []
...
def test_две_реплики_бота_снимается_только_последняя():
    ...
    out = reconcile_last_agent(entries, aired=["Первая"])
    assert len(out) == 1
    assert out[0].text == "Первая"
    assert out[0].role == ROLE_AGENT
...
def test_оклик_после_реплики_не_стирает_её():
    """Оклик после реплики в снимке — запись остаётся, текст не меняется."""
    entry = TranscriptEntry(
        entry_id="a:1:0",
        role=ROLE_AGENT,
        text="Из какого вы города?",
    )
    out = reconcile_last_agent(
        [entry],
        aired=["Из какого вы города?", "Алло, меня слышно?"],
    )
    assert out == [entry]
    assert out[0].text == "Из какого вы города?"


def test_оклик_без_реплики_запись_снимает():
    """В снимке только оклик — запись, которой нет среди реплик, снимается."""
    earlier = TranscriptEntry(entry_id="c:1:0", role=ROLE_CLIENT, text="Алло")
    entries = [
        earlier,
        TranscriptEntry(entry_id="a:1:1", role=ROLE_AGENT, text="Из какого вы города?"),
    ]
    out = reconcile_last_agent(entries, aired=["Алло, меня слышно?"])
    assert out == [earlier]


def test_совпадение_не_на_последнем_месте():
    """Реплика есть в снимке не последней — запись остаётся."""
    entry = TranscriptEntry(entry_id="a:2:1", role=ROLE_AGENT, text="текст записи")
    out = reconcile_last_agent(
        [entry],
        aired=["старая реплика", "текст записи", "оклик"],
    )
    assert out == [entry]


def test_частичная_доставка_берёт_самый_длинный_префикс():
    """Два префикса разной длины — текст урезается до самого длинного."""
    entry = TranscriptEntry(
        entry_id="a:3:2",
        role=ROLE_AGENT,
        text="Добрый день, меня зовут Анна",
...
def test_пустой_список_снимка_историю_не_трогает():
    """Пустой или из пробелов снимок — история без изменений."""
    entries = [
        TranscriptEntry(entry_id="c:1:0", role=ROLE_CLIENT, text="Алло"),
        TranscriptEntry(entry_id="a:1:1", role=ROLE_AGENT, text="Здравствуйте"),
    ]
    assert reconcile_last_agent(entries, aired=[]) == entries
    assert reconcile_last_agent(entries, aired=["", "   "]) == entries


def test_agent_texts_отбирает_только_реплики_бота():
    """Из смеси ролей — только тексты бота, по порядку, пустые отброшены."""
    messages = [
        HumanMessage(content="алло"),
        AIMessage(content="Здравствуйте"),
        AIMessage(content=""),
        HumanMessage(content="да"),
        AIMessage(content="   "),
        AIMessage(content="Из какого вы города?"),
        AIMessage(content="Алло, меня слышно?"),
    ]
    assert agent_texts(messages) == [
        "Здравствуйте",
        "Из какого вы города?",
        "Алло, меня слышно?",
    ]
